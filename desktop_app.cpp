...
bool FlynotyperApp::OnInit()
{
    ...
    wxFont font(10, wxFONTFAMILY_MAX, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL);
    frame->SetFont(font);
    frame->Show();
    return true;
}
...
{   
    // Update count based on user input
    countStr = countInput->GetValue();
    if(countStr != "")
    {
        ncount = std::stoi(countStr);    
    }
    else
    {
        ncount = 200;
    ...
    // Initialize or reset text field that shows flynotyper output
    wxStaticText *m_statictext = flynotyperText;
    m_statictext->SetLabel("Calculating results. Please wait..." + countStr);
    ...
    {   
        ...
        finalFlynotyperText += file + "\t" + output + "\n";
    }
    ...
    std::stringstream ssNewline(finalFlynotyperText);
    ...
    // m_statictext->SetLabel(grid->GetRowSizes().GetSize(0));
